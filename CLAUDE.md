...
- **pdf/**: Native PDF rendering mirroring the `html` component model, with the `codeberg.org/go-pdf/fpdf` engine inlined as root-module source so there is no external PDF dependency (`Document`, text/vector/image primitives, typed enums in `constants.go`). The previous wrapper around the external fpdf module lives on as the separate **`fpdf/`** legacy module, kept as a byte-for-byte parity baseline until it is deleted. See `pdf/README.md` (and `fpdf/README.md` for the legacy module).
