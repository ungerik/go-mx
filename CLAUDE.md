...
- **pdf/**: Native PDF rendering mirroring the `html` component model, with the `codeberg.org/go-pdf/fpdf` engine inlined as root-module source so there is no external PDF dependency (`Document`, text/vector/image primitives, typed enums in `constants.go`, and a best-effort SVG renderer: `pdf.SVG` draws `svg`-package element trees as native vector graphics, visual reference in `pdf/testdata/svg_reference.pdf`). The previous wrapper around the external fpdf module lives on as the separate **`fpdf/`** legacy module, kept as a byte-for-byte parity baseline until it is deleted. See `pdf/README.md` (and `fpdf/README.md` for the legacy module).
- **pkg/pdftable/**: Data tables for `pdf` (`Table` is a retained, two-pass `pdf.Component`): fixed/weighted/auto-measured columns, wrapped cells with a cell→row→column→table style cascade, header rows repeated across automatic page breaks, `Grid` rule combinations, and custom-drawn cells via a `Draw` callback. Visual reference in `pkg/pdftable/testdata/table_reference.pdf`. See `pkg/pdftable/README.md`.
