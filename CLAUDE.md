...
- **pdf/**: Native PDF rendering mirroring the `html` component model, with the `codeberg.org/go-pdf/fpdf` engine inlined as root-module source so there is no external PDF dependency (`Document`, text/vector/image primitives, typed enums in `constants.go`, and a best-effort SVG renderer: `pdf.SVG` draws `svg`-package element trees as native vector graphics, visual reference in `pdf/testdata/svg_reference.pdf`; plus embedded files and document-level XMP metadata via `Document.Attachments`/`Document.XMP` (`XMPMetadata`, `FacturX`, `AFRelationship`) for PDF/A-3 hybrid formats like ZUGFeRD/Factur-X). The previous wrapper around the external fpdf module lives on as the separate **`fpdf/`** legacy module, kept as a byte-for-byte parity baseline until it is deleted. See `pdf/README.md` (and `fpdf/README.md` for the legacy module).
