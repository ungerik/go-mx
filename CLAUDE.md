...
- **html/**: HTML5 elements and attributes - `Div()`, `Span()`, `Class()`, `ID()`, etc. Includes typed keyword enum attributes (`enums.go`, generated by go-enum like `svg`) and numeric attribute constructors that validate at render time. The `html/entity` subpackage provides named character references (`entity.Copyright`, `entity.Heart`, …) as `mx.Raw` constants.
