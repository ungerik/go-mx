...
- **Attribute value types follow the spec.** Each constructor is typed for what
  ...
  its values four per spline (`x1 y1 x2 y2`). `KeySplines` renders an explicit
  error message into the attribute value when the count is not a multiple of
  four, so the SVG fails loudly instead of emitting an invalid spline list.
  (`Class` stays `...string` for multiple class names.) The generic `Attrib`
  constructor remains an escape hatch for any attribute without a dedicated
  function.
- **Strict keyword enums** (attributes whose value is a closed set of keywords —
  ...
  is one of the defined keywords. The generator is pinned in the nested
  [`tools`](../tools) module — kept out of the shipped go-mx dependency tree — and
  run with `go generate ./svg/...`.
